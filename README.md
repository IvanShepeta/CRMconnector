...
This project supports OpenTelemetry tracing. To enable tracing locally:

- Start the AI Toolkit trace collector/viewer in VS Code: run the command `ai-mlstudio.tracing.open`.
- By default the app sends traces to the local OTLP gRPC endpoint `http://localhost:4317`.
- You can customize the endpoint via environment variable `OTEL_EXPORTER_OTLP_ENDPOINT`.
- To include prompts/completions or other sensitive data, set `OTEL_CAPTURE_SENSITIVE=true`.
- The agent framework helper `setup_observability(otlp_endpoint=..., enable_sensitive_data=...)` is used in `AgentCode.py` to initialize tracing and instrumentation.

To generate a test trace locally: 1) Start the AI Toolkit trace collector/viewer (`ai-mlstudio.tracing.open`), 2) Start the MCP server (run the Start MCP Server task), and 3) run `python scripts/generate_trace.py` to trigger an HTTP request and produce spans.
